...
import { $, db } from './db'
import { useFilterSync, omit, pick, table, column, SnakeCaseStrategy, DefaultStrategy, } from 'litdb'
...
describe('SQLite Driver Tests', () => {
    ...
    it ('does CRUD Contact Table', () => {
        ...
        expect(db.value($.from(Contact).where(c => $`${c.age} = 40`).exists())).toBe(false)

        expect(db.value($.from(Contact).exists())).toBe(true)

        db.exec($.deleteFrom(Contact))

        expect(db.value($.from(Contact).exists())).toBe(false)
    ...
    it ('can select column', () => {
        ...
        expect(db.column(q.clone().select(c => $`${c.createdAt}`)).length).toBe(contacts.length)
